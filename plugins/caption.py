...
async def add_caption(client, message):
    ...
    caption = message.text.split(" ", 1)[1]
    await db.set_caption(message.from_user.id, caption=caption)
    await message.reply_text("__**✅ YOUR PREFIX SUCCESSFULLY SAVED**__")

@Client.on_message(filters.private & filters.command('del_prefix'))
async def delete_caption(client, message):
    caption = await db.get_caption(message.from_user.id)  
    if not caption:
       return await message.reply_text("__**😔 YOU DON'T HAVE ANY PREFIX**__")
    await db.set_caption(message.from_user.id, caption=None)
    await message.reply_text("__**❌️ YOUR PREFIX SUCCESSFILLY DELETED**__")

@Client.on_message(filters.private & filters.command('see_prefix'))
async def see_caption(client, message):
    caption = await db.get_caption(message.from_user.id)  
    if caption:
       await message.reply_text(f"**𝚈𝙾𝚄𝚁 PREFIX:-**\n\n`{caption}`")
    else:
       await message.reply_text("__**😔 𝚈𝙾𝚄 𝙳𝙾𝙽𝚃 𝙷𝙰𝚅𝙴 𝙰𝙽𝚈 PREFIX**__")
